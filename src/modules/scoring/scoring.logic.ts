...
function calculateAIScore(findings: AIFinding[]): ScoreDimension {
  ...
  score -= warningCount * 7   // -7 per warning

  // Average confidence adds nuance — low-confidence findings penalize less
  const avgConfidence =
    findings.reduce((sum, f) => sum + f.confidence, 0) / findings.length
  const confidenceModifier = (avgConfidence - 50) / 200 // range -0.25 to +0.25
  ...
  return {
    ...
    score: clampedScore,
    weight: SCORE_WEIGHTS.ai,
    explanation: `${findings.length} issue${findings.length > 1 ? 's' : ''} detected. 
      Primary concern: ${topIssue?.title ?? 'N/A'} (confidence ${topIssue?.confidence ?? 0}%).`,
  }
}
...
function calculateChecklistScore(
  ...
  if (categoryItems.length === 0) {
    return {
      label,
      score: 70, // neutral score if not completed
      weight,
  ...
  return {
    ...
    weight,
    explanation: `${okCount} OK · ${warningCount} warnings · ${problemCount} problems 
      across ${categoryItems.length} items.`,
  }
}
...
function calculateVINScore(
  ...
  if (!hasPremium || !history) {
    return {
      label: 'VIN & History',
      score: 65, // baseline without premium
  ...
  if (history.theftStatus === 'reported_stolen') score -= 100 // instant walk-away flag
  ...
  return {
    ...
    weight: SCORE_WEIGHTS.vin,
    explanation: `${history.accidentCount} accident(s). 
      ${history.recalls.filter((r) => r.status === 'incomplete').length} open recall(s). 
      Mileage: ${mileageConsistent ? 'consistent' : 'inconsistent'}.`,
  }
}
...
function calculateTestDriveScore(
  ...
  return {
    ...
    score: Math.max(0, Math.min(100, Math.round(score))),
    weight: SCORE_WEIGHTS.testDrive,
    explanation: `${goodCount} good · ${concernCount} concerns · ${problemCount} problems 
...
function generateReasons(
  ...
  dimensions: ReturnType<typeof calculateAllDimensions>
  ...
  if (hasPremiumHistory && vinData) {
    ...
    if (vinData.ownershipHistory.length <= 2) for_.push(`Only ${vinData.ownershipHistory.length} previous owners`)
  }
  if (aiFindings.filter((f) => f.severity === 'critical').length === 0) {
    for_.push('No critical AI anomalies detected')
  }
...
/**
 * calculateRiskScore
 * Core scoring function. Takes normalized inspection data, returns complete RiskScore.
 * This is pure logic — no DB calls, no API calls.
 */
export function calculateRiskScore(
  ...
  const buyScore = Math.max(10, Math.min(96, Math.round(weightedSum)))
  const riskScore = 100 - buyScore
  const verdict = determineVerdict(buyScore)
  const { reasonsFor, reasonsAgainst } = generateReasons(input, dimensions)
...
/**
 * getVerdictLabel
 * Returns human-readable verdict with emoji.
 */
